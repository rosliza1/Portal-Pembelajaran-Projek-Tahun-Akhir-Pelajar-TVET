...
2. **Edit `.env`** — set the Supabase connection string:
   ```
   DATABASE_URL=postgresql://postgres:YOUR_PASSWORD@db.wbinwhbmkszeiguwebkl.supabase.co:5432/postgres
   ```
   *(Note: URL-encode special characters in your password — `@` → `%40`)*
...
1. **Password Security**: Your database password is stored in `.env` (gitignored). Consider changing it in Supabase Dashboard → Project Settings → Database → Reset password for production.
...
3. **Connection Pooler**: For production, Supabase recommends using the connection pooler (port 6543) instead of direct connection (port 5432) to avoid exhausting connection limits. Update the URL to:
   ```
   postgresql://postgres.wbinwhbmkszeiguwebkl:YOUR_PASSWORD@aws-0-ap-southeast-1.pooler.supabase.com:6543/postgres
   ```
